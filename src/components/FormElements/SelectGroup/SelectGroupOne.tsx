...
type SelectGroupOneProps = {
  label: string;
  name: string;
  value: string | string[]; // Support single and multi-select
  onChange: (e: React.ChangeEvent<HTMLSelectElement>) => void;
  options: Option[];
  multiple?: boolean; // New prop for multi-select support
};
...
const SelectGroupOne: React.FC<SelectGroupOneProps> = ({
  ...
  const isOptionSelected = multiple
    ? Array.isArray(value) && value.length > 0 // Multi-select check
    : value !== ""; // Single-select check
    ...
    <div className="mb-4.5">
      ...
      <div className="relative z-20 bg-transparent dark:bg-dark-2">
        <select
          id={name}
          name={name}
          value={multiple ? (value as string[]) : (value as string)} // Handle multi-select values
          onChange={onChange}
