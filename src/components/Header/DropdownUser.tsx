...
const DropdownUser = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try{
        const data = await fetch("/api/user/hydrate")
        if (data.ok){
  ...
  const handleLogout = async () => {
    ...
    if (!res.ok){
      toast.error("Error performing operation. Please try again later.")
    }else{
  ...
  if(userData === null){
    return(
      ...
      <Link
        ...
        href="#"
      >
        <span className="h-12 w-12 rounded-full object-cover aspect-square">
          <Image
            width={112}
            height={112}
            src={profilePic}
          ...
          <div className="flex items-center gap-2.5 px-5 pb-5.5 pt-3.5">
            ...
            <span className="block">
              <span className="block font-medium text-dark dark:text-white">
                {userData?.name || "Jhon Smith"}
              </span>
              <span className="block font-medium text-dark-5 dark:text-dark-6">
                {userData?.email || "jonson@nextadmin.com"}
              </span>
            </span>
          </div>
          <ul className="flex flex-col gap-1 border-y-[0.5px] border-stroke p-2.5 dark:border-dark-3">
